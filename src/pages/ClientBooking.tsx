                  ...
                  <div className="space-y-2">
                    {servicesForProfessional(selectedProfessional?.id || "")
                      ...
                      <motion.button key={s.id}
                        ...
                        className="w-full flex items-center gap-3 p-3.5 bg-white border border-zinc-100 rounded-2xl hover:border-zinc-200 hover:shadow-sm transition-all active:scale-[0.98] text-left group">
                        <div className="w-9 h-9 rounded-full overflow-hidden shrink-0 flex items-center justify-center" style={{ backgroundColor: s.photo ? undefined : customColor + "12" }}>
                          {s.photo ? <img src={s.photo} alt={s.name} className="w-full h-full object-cover" /> : <Scissors size={14} style={{ color: customColor }} />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-display text-[14px] font-semibold text-zinc-800 leading-tight tracking-tight">{s.name}</p>
                          <p className="text-[11px] text-zinc-400 font-medium mt-0.5 flex items-center gap-1">
                            <Clock size={9} className="shrink-0" />{s.duration} min
                          </p>
                        </div>
                        {publicAgendaSettings.selfServiceShowPrices && (
                          <p className="font-display text-[13px] font-bold shrink-0 whitespace-nowrap tabular-nums" style={{ color: customColor }}>
