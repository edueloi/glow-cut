...
export interface GenerateNfsePdfParams {
  ...
  substitutedChaveAcesso?: string | null;
 ...
 * da nota, logo do salão, QR Code e código de barras da chave de acesso, para o
 * cliente/salão imprimir ou guardar).
 */
export async function generateNfsePdf({
  logoBuffer,
...
}: GenerateNfsePdfParams): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const colorPrimary = "#1e40af";
    const colorPrimaryLight = "#eff6ff";
    const colorPrimaryBorder = "#bfdbfe";
    const colorText = "#1e293b";
    const colorMuted = "#64748b";
    const colorBorder = "#e2e8f0";
    const colorSoftBg = "#f8fafc";
    ...
    // Altura real do cabeçalho: a maior entre o bloco de texto (o nome do salão
    ...
    type CardLine =
      ...
      | { type: "row"; fields: { label: string; value?: string | null }[] };
    ...
    const cardPad = 8;
    function drawCard(lines: CardLine[]) {
    ...
    function drawTwoColumnCards(
      leftTitle: string, leftLines: { label: string; value?: string | null }[],
      rightTitle: string, rightLines: { label: string; value?: string | null }[]
    ) {
      const gap = 12;
      ...
      function measure(lines: { label: string; value?: string | null }[]) {
      ...
      function draw(measured: { label: string; value?: string | null; height: number }[], x: number) {
        let cy = boxY + pad;
        measured.forEach((line) => {
