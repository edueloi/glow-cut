...
import { onlyDigits } from "./dpsId";

// Monta "AAAA-MM-DDTHH:mm:ss±HH:mm" na hora LOCAL do processo — mesmo formato exigido
// para dhEmi da DPS (ver dpsXmlBuilder.ts); o layout nacional rejeita UTC puro com "Z".
...
interface GerarIdPedidoEventoParams {
  chaveAcesso: string;
  tipoEvento: number;
  sequencial?: number;
}
...
export function gerarIdPedidoEvento({ chaveAcesso, tipoEvento, sequencial = 1 }: GerarIdPedidoEventoParams): string {
...
export interface EventoEmitter {
  cnpj?: string | null;
  environment: string;
}
...
export function buildCancelamentoXml({ emitter, chaveAcesso, motivo }: BuildCancelamentoXmlParams): { idPedido: string; xml: string } {
